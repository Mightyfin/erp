...
export const Route = createFileRoute("/requests/new")({
  head: () => ({
    meta: [
      { title: "Raise an HR request — Meridian ERP HRM" },
      { name: "description", content: "Guided case submission with purpose, details, evidence and next steps." },
      { property: "og:title", content: "Raise an HR request — Meridian ERP HRM" },
      { property: "og:description", content: "Guided case submission with purpose, details, evidence and next steps." },
    ],
  }),
...
const suggestedAnswers: Record<string, string> = {
  ...
  "Payroll query": "Payslip explanations are available directly on each payslip — check there before raising a case.",
...
function NewRequest() {
  ...
  const steps: FlowStep[] = [
      ...
      render: () => (
        <div className="max-w-lg space-y-4">
          ...
          <div>
            ...
            <Input id="subject" className="mt-1" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Short summary" />
          </div>
        </div>
      ),
    },
      ...
      render: () => (
        <div className="max-w-lg space-y-5">
          <fieldset>
            ...
            <RadioGroup value={confidentiality} onValueChange={setConfidentiality} className="mt-2 space-y-2">
              <label className="flex items-center gap-2 text-sm"><RadioGroupItem value="standard" /> Standard — visible to HR operations and your manager where relevant</label>
              <label className="flex items-center gap-2 text-sm"><RadioGroupItem value="restricted" /> Restricted — visible to HR operations only</label>
            </RadioGroup>
          </fieldset>
          <fieldset>
            ...
            <RadioGroup value={contact} onValueChange={setContact} className="mt-2 space-y-2">
              <label className="flex items-center gap-2 text-sm"><RadioGroupItem value="app" /> In this app only</label>
              <label className="flex items-center gap-2 text-sm"><RadioGroupItem value="email" /> Email me updates too</label>
            </RadioGroup>
          </fieldset>
        </div>
      ),
    },
    {
      ...
      purpose: "Check the facts. Submitting creates one case thread you can track from HR requests.",
      render: () => (
        <dl className="grid max-w-lg gap-3 sm:grid-cols-2">
          {[["Category", category], ["Subject", subject || "Not given"], ["Confidentiality", confidentiality === "restricted" ? "Restricted" : "Standard"], ["Contact", contact === "email" ? "In-app + email" : "In-app only"]].map(([k, v]) => (
        ...
        submitted={
          ref ? (
            <NextSteps
              ...
              actions={<><Button onClick={() => navigate({ to: "/requests" })}>View my requests</Button><Button variant="outline" asChild><Link to="/">Back to Home</Link></Button></>}
            />
          ) : undefined
        }
      />
