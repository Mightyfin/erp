...
async function loadHome(role: string) {
  ...
  for (const item of rows(workflow)) {
    ...
    items.push({
      ...
      context: `${String(item.subjectName ?? "Worker")} · ${String(item.status ?? "submitted")}`,
  ...
  for (const item of rows(corrections)) {
    items.push({
      ...
      context: `${String(item.workerName ?? "Worker")} · ${String(item.workDate ?? "")}`,
