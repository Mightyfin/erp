...
export const Route = createFileRoute("/attendance/")({
  head: () => ({
    meta: [
      { title: "Attendance corrections — Meridian ERP HRM" },
      { name: "description", content: "Review and correct clocking exceptions with a full audit trail." },
      { property: "og:title", content: "Attendance corrections — Meridian ERP HRM" },
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        primaryAction={
          <Button asChild>
            <Link to="/attendance/new">Raise a correction</Link>
          </Button>
        }
      />
      <Async state={state}>
        {(rows) => (
          <ListPage<AttendanceCorrection>
            ...
            columns={[
              { id: "ref", header: "Reference", cell: (r) => <Link to="/attendance/$id" params={{ id: r.id }} className="font-mono text-xs text-primary underline underline-offset-2">{r.id}</Link> },
