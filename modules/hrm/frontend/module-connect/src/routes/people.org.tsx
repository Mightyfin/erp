...
export const Route = createFileRoute("/people/org")({
  head: () => ({
    meta: [
      { title: "Organisation structure — Meridian ERP HRM" },
      {
        name: "description",
        content:
          "Indented tree of entities, branches, departments and teams with headcount, vacancies and the lead of each unit.",
      },
      { property: "og:title", content: "Organisation structure — Meridian ERP HRM" },
      {
  ...
  return (
    <aside aria-label="Selected unit" className="rounded-lg border bg-surface p-5">
      ...
      <div className="mt-4 space-y-2 text-sm">
        <Link to="/people/positions" className="block text-primary underline underline-offset-2">
          Positions in the establishment
        </Link>
        <Link to="/employees" className="block text-primary underline underline-offset-2">
          Employee directory
        </Link>
      </div>
    </aside>
