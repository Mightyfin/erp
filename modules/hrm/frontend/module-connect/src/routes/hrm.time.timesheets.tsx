...
import { ChevronDown, ChevronLeft, ChevronRight, Clock3, Download, Edit3, FileClock, LockKeyhole, MoreHorizontal, Plus, Search, TimerReset, UserRound } from "lucide-react";
...
function TimesheetsPage() {
  ...
  const dayGroups = useMemo<DayGroup[]>(() => {
    ...
    return DAYS.map((day, index) => {
      const entries = timesheet.rows
        ...
        .filter((entry) => entry.total > 0 && `${entry.project} ${entry.detail}`.toLowerCase().includes(query.trim().toLowerCase()));
  ...
  }, [query, timesheet, weekDates]);

  const totals = useMemo(() => ({
    total: dayGroups.reduce((sum, day) => sum + day.total, 0),
    overtime: dayGroups.reduce((sum, day) => sum + day.overtime, 0),
    attention: dayGroups.reduce((sum, day) => sum + day.entries.filter((entry) => entry.status !== "Recorded").length, 0),
  }), [dayGroups]);
  ...
  return <AuthGate><AppShell><PageHeader eyebrow="Time and leave / work records" title="Timesheets" description="See your working time by day, keep the week complete, and resolve anything that needs attention." meta={<Badge variant="outline" className="gap-1.5 border-info/30 bg-info-soft text-info-foreground"><LockKeyhole className="size-3" aria-hidden /> Frontend workflow preview</Badge>} primaryAction={<Button onClick={openAddTimer} className="gap-2"><Plus className="size-4" aria-hidden />Add time entry</Button>} />
    <div className="space-y-5" data-testid="timesheets-page">
      ...
      {timesheet ? <><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Your week</h2><p className="text-sm text-muted-foreground">Entries are grouped by day so missing or unusual time is easy to spot.</p></div><div className="relative hidden sm:block"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search work items" className="w-64 pl-9" aria-label="Search work items" /></div></div><div className="sm:hidden"><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search work items" aria-label="Search work items" /></div><div className="space-y-3">{(view === "daily" ? dayGroups.filter((day) => day.date === format(new Date(), "yyyy-MM-dd")) : dayGroups).map((day) => <DaySection key={day.date} day={day} open={expanded[day.date] ?? true} onToggle={() => setExpanded((current) => ({ ...current, [day.date]: !(current[day.date] ?? true) }))} />)}</div></> : null}
