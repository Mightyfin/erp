...
export const Route = createFileRoute("/requests/")({
  head: () => ({
    meta: [
      { title: "HR requests — Meridian ERP HRM" },
      { name: "description", content: "Raise and track HR cases: letters, data changes and queries." },
      { property: "og:title", content: "HR requests — Meridian ERP HRM" },
      ...
      <Async state={state}>
        {(rows) => (
          <ListPage<HrCase>
            ...
            columns={[
              { id: "ref", header: "Reference", cell: (r) => <Link to="/requests/$id" params={{ id: r.id }} className="font-mono text-xs text-primary underline underline-offset-2">{r.id}</Link> },
