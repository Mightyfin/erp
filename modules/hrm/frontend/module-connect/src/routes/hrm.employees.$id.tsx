...
function EmployeePage() {
  ...
  const printCorrectionStatement = async (workerId: string, employeeNo: string, employeeName: string) => {
    ...
    try {
      ...
      const html = `<!doctype html><html><head><title>Mightyfin HRMS payroll correction statement</title><style>@page{margin:16mm}body{font:13px Arial,sans-serif;color:#17212b;margin:0}.brand{background:#410064;color:#fff;padding:22px 24px;border-bottom:6px solid #fec00f}.brand strong{font-size:22px;display:block}.brand span{font-size:11px;opacity:.88}.document{margin:24px}.document h1{font-size:19px;color:#410064;margin:0 0 4px}.muted{color:#59636d;font-size:11px}table{width:100%;border-collapse:collapse;margin:18px 0}td,th{border:1px solid #cfd6dc;padding:8px;text-align:left}th{background:#f2edf6;color:#410064}.r{text-align:right}.total th{background:#410064;color:#fff;font-size:14px}.notice{border-left:4px solid #fec00f;background:#fff9df;padding:9px 12px;font-size:11px}</style></head><body><header class="brand"><strong>Mightyfin HRMS</strong><span>Human Resources &middot; Payroll Correction Statement</span></header><main class="document"><h1>Payroll Correction Statement</h1><p class="muted">This document supplements, and does not replace, the released payslip.</p><table><tr><td>Employee</td><td>${escapeHtml(employeeName)}</td><td>Employee no.</td><td>${escapeHtml(employeeNo)}</td></tr><tr><td>Period</td><td>${escapeHtml(slip.periodLabel)}</td><td>Original payslip</td><td>${escapeHtml(slip.payslipNo)}</td></tr></table><table><tr><th>Description</th><th class="r">Amount (ZMW)</th></tr><tr><td>Original released net pay</td><td class="r">${Number(slip.net ?? 0).toFixed(2)}</td></tr>${rows.map((row) => `<tr><td>${escapeHtml(row.component ?? "Payroll adjustment")}</td><td class="r">${Number(row.netAdjustmentPaid ?? row.grossAdjustment ?? 0).toFixed(2)}</td></tr>`).join("")}<tr class="total"><th>Adjusted payment total</th><th class="r">${(Number(slip.net ?? 0) + adjustment).toFixed(2)}</th></tr></table><p class="notice">Generated from the HRM audit trail on ${new Date().toLocaleString()}. Statutory treatment remains recorded on the original payroll and any approved supplementary payroll.</p></main><script>window.onload=()=>window.print()</script></body></html>`;
