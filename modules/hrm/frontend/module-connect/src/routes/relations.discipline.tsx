...
export const Route = createFileRoute("/relations/discipline")({
  head: () => ({
    meta: [
      { title: "Warning register — Meridian ERP HRM" },
      { name: "description", content: "Disciplinary warnings, when they lapse, and why a lapsed warning cannot be relied on." },
      { property: "og:title", content: "Warning register — Meridian ERP HRM" },
          ...
          <ListPage<Warning>
            ...
            columns={[
              ...
              { id: "appeal", header: "Appeal", cell: (w) => <span className="text-xs">{w.appeal}</span> },
              {
                ...
                cell: (w) =>
                  w.caseId ? (
                    <Link to="/relations/cases/$id" params={{ id: w.caseId }} className="font-mono text-xs text-primary underline underline-offset-2">
                      {w.caseId}
                    </Link>
                  ) : (
