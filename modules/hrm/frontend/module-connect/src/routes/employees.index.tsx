  ...
  const columns: ColumnDef<Employee>[] = [
    {
      ...
      cell: (e) => (
        <div className="min-w-0 max-w-64">
          <Link to="/employees/$id" params={{ id: e.id }} className="block truncate font-medium text-primary underline-offset-2 hover:underline">
            {e.fullName}
          </Link>
      ...
      <Async state={state} rows={6}>
        {(rows) => (
          <ListPage
            ...
            rowHref={(e) => (
              <Link to="/employees/$id" params={{ id: e.id }} className="text-xs font-medium text-primary underline underline-offset-2">
                Open
              </Link>
            )}
          />
