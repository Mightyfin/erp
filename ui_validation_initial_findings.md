...
## Frontend-first Timesheets UX redesign — 2026-08-23

The prior mixed Time Operations/Timesheets experience was reconsidered against the supplied TimeSheet.io reference. The main lesson is that timesheets should be a focused work-record workspace rather than a card wall combining imports, shift configuration, leave operations, escalation and overtime decisions.

The live `/hrm/time/timesheets` route was redesigned frontend-first with a period toolbar, previous/next week navigation, Weekly/Daily view selection, Download entry point, four truthful summary tiles, inline Add time entry interaction, and a day-grouped record area reserved for live rows. In production mode, the existing mock guard prevents demo rows; the page displays zero values and a clear “Timesheets are ready for live connection” message instead of inventing projects or hours. The Add time entry interaction explicitly states that it is a frontend-only draft and writes no production record until the live timesheet service is connected.

The release scope no longer routes Timesheets to the Coming Soon screen. Attendance import is a separate route using the shared ImportDialog and shared ExportButton. Overtime review remains separate at `/hrm/time/operations`. The shared Import/Export hub is available at `/hrm/data/import-export`. The next frontend work should continue separating shift rules, shift assignments, leave operations, and attendance corrections into their own pages without adding API behavior prematurely.

Browser validation confirmed the live Timesheets route, week navigation from 23–29 August 2026 to 30 August–5 September 2026, Daily view selection, summary tiles, empty state, and Add time entry interaction. The production web image was rebuilt and deployed without changing API or PostgreSQL services. The frontend build passed; the full repository typecheck still has unrelated baseline errors outside the redesigned routes.
